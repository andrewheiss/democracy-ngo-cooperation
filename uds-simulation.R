...
p <- ggplot(coefs.plot, aes(y=estimate, x=coef.name, fill=coef.name)) + 
  geom_violin(scale="width") + geom_hline(yintercept=0) + coord_flip() + 
  labs(x=NULL, y="Coefficient estimate (log odds)") + theme_bw(8) + 
  guides(fill=FALSE)
ggsave(p, filename="Figures/simulated_uds.pdf", width=6, height=3.5)
